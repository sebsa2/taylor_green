...
! into Taylor_Green vortex
    ...
    allocate(U(P))
    allocate(V(P))
    allocate(D(nb_outputs_allpart,density_matrix_size,density_matrix_size))

    ! Create name of the output files
    write (filename_output, '("output_density_matrix_St",I4,"_P",I10,"_Tf",I3,"_dt",I3,".txt")' )&
    ...
    V(:) = 0
	do j=1,P,1
		D(counter, floor(density_matrix_size*X(j))+1, floor(density_matrix_size*Y(j))+1) &
        = 1 + D(counter, floor(density_matrix_size*X(j))+1, floor(density_matrix_size*Y(j))+1)
	enddo
